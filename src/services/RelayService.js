...
export class RelayService
{
	...
	async createRelay( options )
	{
		return new Promise( async ( resolve, reject ) =>
		{
			try
			{
				if ( ! ProcessUtil.isValidPortNumber( options.port ) )
				{
					options.port = LocalParamUtil.getDefaultPort();
				}
				...
				{
					return reject( `${ this.constructor.name } :: failed to create/load peerId. Create a new peerId using [debeem-utils]` );
				}
				...
				if ( null === swarmKey )
				{
					return reject( `${ this.constructor.name } :: invalid swarm key. Create a new swarm key using [debeem-utils]` );
				}
				...
				const createP2pOptions = CreateP2pOptionsBuilder.builder()
					...
					.setPubsubPeerDiscoveryTopics( options.pubsubPeerDiscoveryTopics )
					.setCallbackMessage( ( param ) =>
					{
						this.onReceivedMessage( param );
					} )
					.build();
				LogUtil.say( `pubsubPeerDiscoveryTopics: ${ options.pubsubPeerDiscoveryTopics.map( t => t ) }` );
